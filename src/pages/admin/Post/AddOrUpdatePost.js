...
import { addPost , updatePost , getPostById , getAllTypePost, uploadImage, getEmployeeByUserId } from "utils/api";
    ...
    return ( 
        <AddOrUpdateStyles>
            ...
            <div className="right-layout">
                <button onClick={()=>{
                    handleSave()
                }} className="btn-order handle">{id != '0' ? "Cập nhật" : "Tạo"} bài viết</button>
            </div>
            <div className="box-container">
                <div className="img-box">
                    <label className="title-input" for="img">Thumbnail</label>
                    <img src={post.AnhNen ? post.AnhNen : "https://res.cloudinary.com/dbar5movy/image/upload/v1683052842/RestaurantManagement/rttafiiipag4estaeuas.jpg"} />
                    <input hidden type="file" name="img" id="img" onChange={(e)=>{
                        handleChangeImage(e)
                    }}/>
                    <label className="btn-order info mt-1" for="img">Chọn ảnh</label>
                    {errImg && <span className="message">{errImg}</span>}
                </div>
                <div className="img-text">
                    <div >
                        <label className="title-input" for="title">Tiêu đề</label>
                        <textarea name="title" id="title" rows={5} value={post.TieuDe} onChange={(e)=>{
                    ...
                    <div >
                        ...
                        <CustomDropDown selectedItem={selectedItem} typePosts={typePosts} setSelectedItem={setSelectedItem}/>
            ...
            <div>
                ...
                init={{
                        ...
                        toolbar: 'undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table mergetags | addcomment showcomments | spellcheckdialog a11ycheck typography | align lineheight | checklist numlist bullist indent outdent | emoticons charmap | removeformat | paste | fontsizeselect',
                        ...
                        fontsize_formats: "8px 10px 12px 13px 14px 15px 16px 17px 18px 19px 20px 21px 22px 23px 24px 25px 26px 27px 28px 29px 30px 31px 32px 33px 34px 35px 36px 38px 40px 42px 44px 50px",
                        font_formats:
